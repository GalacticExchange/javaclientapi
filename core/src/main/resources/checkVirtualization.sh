...
# source code link http://bazaar.launchpad.net/~cpu-checker-dev/cpu-checker/trunk/view/head:/kvm-ok
...
verdict() {
	# Print verdict
	if [ "$1" = "0" ]; then
		echo "Hardware acceleration can be used"
		exit 0
	else
		echo "Hardware acceleration can NOT be used"
		exit 1
	fi
}
...
if [ -z "$virt" ]; then
	echo "INFO: Your CPU does not support virtualization"
	assert_root
	verdict 1
fi
...
if [ "$disabled" -eq 1 ]; then
	echo "INFO: Virtualization ($virt) is disabled by your BIOS"
